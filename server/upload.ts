...
export async function processUpload(
  ...
  for (let i = 0; i < rows.length; i++) {
    try {
      ...
      const name = getField(row, "valoroperacao", "nome", "name", "cliente", "beneficiario");
      if (!name) continue;

      const phone = cleanPhone(getField(row, "telefones.1", "telefone", "phone", "celular", "whatsapp", "fone"));
      const cpf = getField(row, "produto", "cpf", "documento");
      const proposta = getField(row, "banco", "proposta", "numeroproposta", "numproposta");
      const banco = getField(row, "dataatualizacao", "banco", "bank", "instituicao");
      const statusRaw = getField(row, "telefones", "status", "situacao");
      const notes = getField(row, "obs.1", "obs", "observacao", "observacoes", "notes");
      const vendedor = getField(row, "vendedor") || null;
