...
export const appRouter = t.router({
  ...
  deleteClient: t.procedure
    .input(z.object({ id: z.number() }))
    .mutation(async ({ input }) => {
      await db.update(clients).set({ active: false, updatedAt: new Date() })
  ...
  webhook: t.procedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (["message.sent","message.delivered","message.read","delivery","read"].includes(event))
        return { ok: true };
  ...
  getConfig: t.procedure.query(async () => {
    ...
    return {
      zapi_instance_id:  fromDb.zapi_instance_id  || process.env.ZAPI_INSTANCE_ID  || "",
      zapi_token:        fromDb.zapi_token         || process.env.ZAPI_TOKEN         || "",
      zapi_client_token: fromDb.zapi_client_token  || process.env.ZAPI_CLIENT_TOKEN  || "",
      report_phone:      fromDb.report_phone       || process.env.REPORT_PHONE       || "",
      ...fromDb,
    };
  }),
